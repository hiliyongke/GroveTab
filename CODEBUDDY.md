...
```tsx
// ✅ 正确：缓存昂贵计算
const sortedTabs = useMemo(() => tabs.sort((a, b) => b.lastAccessed - a.lastAccessed), [tabs]);
...
const TabCard = React.memo(
  ({ tab, onClose }: TabCardProps) => {
    return <Card onClick={() => handleTabClick(tab.id)}>{tab.title}</Card>;
  },
  (prevProps, nextProps) => {
    return prevProps.tab.id === nextProps.tab.id && prevProps.tab.title === nextProps.tab.title;
  },
);
...
// 使用
const { execute, loading, error } = useAsyncTask(() => chrome.tabs.query({}), {
