 ...
 * 组合 key，变化即触发派生重算
 *
 * 将 favicon URL 和 colorKey 组合成唯一的字符串 key，
 * 用于检测依赖项是否变化。
 *
 * @param favicon - favicon URL（可能未定义）
 * @param colorKey - 兜底色键（通常是注册域）
 * @returns 组合后的 key 字符串
 */
...
/**
 * 根据 favicon/colorKey 立即可得的强调色
 *
 * 同步解析强调色：
 *   - 如果 favicon 已有缓存（成功或已知失败），直接返回缓存结果
 *   - 否则返回基于 colorKey 的哈希兜底色
 *
 * @param favicon - favicon URL（可能未定义）
 * @param colorKey - 兜底色键（通常是注册域）
 * @returns 解析后的强调色（Accent 对象）
 */
 ...
 *
 * 优先从 favicon 提取主色；提取失败回退到基于 colorKey 的哈希色。
 * 结果自动缓存，组件重渲染时避免重复计算。
 *
 * @param favicon favicon URL（可空）
 * @param colorKey 兜底色键（通常是注册域）
 * @returns 强调色对象（Accent）
 */
