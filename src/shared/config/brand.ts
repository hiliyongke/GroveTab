 ...
 */
const BRAND_PRESETS: Record<string, BrandIdentity> = {
  groveTab: {
    id: 'groveTab',
    name: 'GroveTab',
    shortName: 'G',
...
const ACTIVE_BRAND_ID = (() => {
  const fromEnv = typeof import.meta !== 'undefined'
    ? (import.meta as unknown as { env?: { VITE_BRAND?: string } }).env?.VITE_BRAND
    : undefined;
 ...
 */
export function getBrandDisplayName(locale: string): string {
  if (locale === 'en') return BRAND.name;
 ...
 */
export function getBrandTagline(locale: string): string {
