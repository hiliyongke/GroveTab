...
function createCustomEngineId(
  label: string,
  existing: LocalCustomSearchEngine[],
): `custom:${string}` {
  const slug = label
    .trim()
    .toLowerCase()
...
export function SearchSettings({ settings, updateSettings }: SearchSettingsProps) {
  ...
  const enabledEngines = normalizeEnabledSearchEngines(
    settings.searchEnabledEngines,
    customEngines,
  );
  const defaultSearchEngine: SearchEngineId = enabledEngines.includes(
  ...
  return (
    <Flex vertical className="settings-panel-stack settings-panel-stack--regular">
      <Field label={t("搜索范围")} hint={t("选择搜索时匹配哪些字段；缩小范围可提升搜索速度")}>
        <Checkbox.Group
      ...
      <Field label={t("默认搜索引擎")} hint={t("在搜索框中回车时使用的搜索引擎")}>
      ...
      <Field label={t("启用的搜索引擎")} hint={t("选择搜索框下方快捷切换的搜索引擎")}>
        <Checkbox.Group
          className="settings-checkbox-group"
