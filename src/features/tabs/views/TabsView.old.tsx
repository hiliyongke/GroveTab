...
import { consumeInsightsDomainFilter } from "@/shared/utils/insights-filter";

export const TabsView = memo(function TabsView() {
  const layout = useSettingsStore(
    useShallow((s) => s.settings.tabsLayout ?? "masonry"),
  );
  // P1-06: 从 Insights 点击域名跳转时，预填搜索框
  const [filterQuery, setFilterQuery] = useState(() => consumeInsightsDomainFilter() ?? "");
